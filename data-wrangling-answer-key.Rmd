---
title: "POLS 7012 Week 4: Data Wrangling (Answer Key)"
author: "Joe Ornstein"
...
> **Exercise:**
>
> You should have a dataframe called `cars` available to you (it comes with base `R`; just type cars into the console to see it). Write the `cars` dataframe to a csv file in your data/ folder called `cars.csv`. Try to to open that file with any spreadsheet software, like Excel. Then read it back into an object called `cars2`. 
...
```{r read stata exercise}
library(readstata13) # load library to read stata files

# read from stata data file
hall <- read.dta13('data/primary_analysis.dta')

# write it to a CSV (you should be able to open that new file in Excel)
write_csv(hall, 'data/primary_analysis.csv')
```
...
# read Chinese GDP data
chinese_gdp <- read_csv('data/chinese_gdp.csv')

chinese_gdp

# left_join with tb_data on the left
# It keeps all the rows from tb_data, and leaves empty all the GDP values where it can't find matches on the key variables
tb_data %>% 
  left_join(chinese_gdp, by = c('country', 'year'))


# left_join with chinese_gdp on the left
# keeps all of the GDP data, leaves cases & population missing where it can't find a matching key
chinese_gdp %>% 
  left_join(tb_data, by = c('country', 'year'))
...
```{r filter with %in%}
# keep only the rows corresponding to Afghanistan and China
tb_data %>% 
  filter(country %in% c('Afghanistan', 'China'))
```
...
# here's a list of the country codes
wvs$B_COUNTRY_ALPHA %>% 
  unique %>% 
  sort

wvs %>% 
  filter(B_COUNTRY_ALPHA %in% c('USA', 'MEX')) %>% 
  select(B_COUNTRY_ALPHA, Q150, Q152) %>% 
  head(10)
```
...
```{r merge with NMC exercise}

# load NMC
nmc <- read_csv('data/NMC_5_0.csv') %>% 
  filter(year == 2012) %>% # keep the most recent year
  select(ccode, milex) # keep only the country identifier and military expenditures

# make sure the country code has a consistent name across datasets
wvs$ccode <- wvs$C_COW_NUM

# merge by ccode
wvs <- wvs %>% 
  left_join(nmc, by = 'ccode')

wvs %>% 
  select(B_COUNTRY_ALPHA, Q151, milex) %>% 
  head(10)
```
...
```{r mutate}
tb_data <- tb_data %>% 
   mutate(people_without_tuberculosis = population - cases,
...
# positive immigration perceptions is the sum of Q122 through Q129
# but use 2-X for the variables where 2 represents a negative perception
wvs <- wvs %>% 
  mutate(positive_immigration_perceptions = 
           Q122 + Q123 + (2 - Q124) + Q125 + (2 - Q126) + 
           Q127 + (2 - Q128) + (2 - Q129))

# here's what the distribution of that new variable looks like
# 16 is the most positive perception of immigrants; 0 is the least positive
ggplot(wvs) +
  geom_histogram(aes(x=positive_immigration_perceptions))
...
case_when(x == 1 ~ 'Puppies', # when x == 1, return "Puppies"
          x == 2 ~ 'Kittens', # when x == 2, return "Kitties"
          x > 7 ~ 'Otter Pups', # when x > 7, return "Otter Pups"
          TRUE ~ "Other Baby Animals") # for all other values of x, return "Other Baby Animals"
```
...
```{r case_when exercise}

# returns NA for elements in x that don't satisfy any of the conditions
...
```{r recoding}
data <- data %>% 
  mutate(libcon = case_when(lcself %in% 1:2 ~ 'Liberal',
                            lcself %in% 3:5 ~ 'Moderate',
                            lcself %in% 6:7 ~ 'Conservative'))
...
> **Exercise 2**
>
> Using the ANES Pilot data, create a scatter plot of feeling thermometer towards legal immigrants vs. illegal immigrants, faceted by party. Filter out the non-partisans. Make sure to recode partisanship so that it looks nice, and use pipes.

```{r piping into a ggplot exercise}
data %>% 
  mutate(partisanship = case_when(vote20dem == 1 ~ 'Democrat',
                                  vote20dem == 2 ~ 'Republican',
                                  vote20dem == 3 ~ 'Neither')) %>% 
  filter(partisanship %in% c('Democrat', 'Republican'),
         ...
         ftimmig2 %in% 1:100) %>%
  ggplot + 
  geom_point(mapping = aes(x=ftillegal, y=ftimmig2)) +
  facet_wrap(~partisanship) +
  labs(title = 'Feelings Towards Immigrants',
       x = 'Illegal Immigrants', y = 'Legal Immigrants')
```
...
data <- data %>% 
  ...
  left_join(state.fips, by = 'fips')

data %>% 
  group_by(abb) %>% 
  summarize(mean_religious_imporantance = mean(pew_religimp),
            num_observations = n()) %>% 
  arrange(mean_religious_imporantance) %>% 
  kable
```
